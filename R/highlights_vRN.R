...
HL_FILE <- Sys.getenv("SGH_HL_FILE",
                      unset = if (!is.null(.HL_ROOT)) file.path(.HL_ROOT, "analysis", "03_mainline.Rmd")
                              else file.path("analysis", "03_mainline.Rmd"))
HL_TAGS <- c("KEY", "NULL", "PARAM", "DECISION", "CAVEAT", "TODO")
HL_OUT  <- if (!is.null(.HL_ROOT)) file.path(.HL_ROOT, "outputs", "script_highlights.md")
           else file.path("outputs", "script_highlights.md")
