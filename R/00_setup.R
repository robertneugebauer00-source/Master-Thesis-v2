...
## [repo 18.09.2026] Everything lives in this repository by default:
##   inputs  -> <repo>/data-raw/   (flat folder)
##   outputs -> <repo>/outputs/
## The repo root is found automatically, with no machine-specific paths:
##   1. from the location of THIS file (it lives in <repo>/R/), when sourced;
##   2. else by walking up from the working directory to the first folder that
##      looks like the repo (any of: *.Rproj, .git, run_all.R, R/00_setup.R);
##   3. else the working directory itself (with a warning).
## So the repo can be moved, renamed or cloned anywhere, on any OS. To read inputs
## from an external data tree instead (old layout: scripts/, raw_data/, ...),
## set the SGH_PROJ environment variable; outputs still go to <repo>/outputs/.
.is_repo_root <- function(d) {
  length(list.files(d, pattern = "\\.Rproj$")) > 0 ||
    dir.exists(file.path(d, ".git")) ||
    file.exists(file.path(d, "run_all.R")) ||
    file.exists(file.path(d, "R", "00_setup.R"))
}
REPO <- local({
  ## 1. where is this file? (set by source(); NULL when run line-by-line)
  this <- NULL
  for (i in rev(seq_len(sys.nframe()))) {
    of <- sys.frame(i)$ofile
    if (!is.null(of) && basename(of) == "00_setup.R") { this <- of; break }
  }
  if (!is.null(this)) {
    d <- dirname(dirname(normalizePath(this, winslash = "/", mustWork = TRUE)))
    if (.is_repo_root(d)) return(d)
  }
  ...
  warning("[setup] could not locate the repo root; using the working directory: ", getwd())
  normalizePath(getwd(), winslash = "/")
})
.ext_proj <- nzchar(Sys.getenv("SGH_PROJ"))
proj      <- if (.ext_proj) Sys.getenv("SGH_PROJ") else file.path(REPO, "data-raw")
## in_file(): path to an input -- flat in data-raw/, or <sub>/<name> in an external tree
in_file <- function(name, sub = NULL) {
  if (.ext_proj && length(sub)) do.call(file.path, as.list(c(proj, sub, name))) else file.path(proj, name)
}
conc_file <- in_file("Finckh_Carmona_2023_PANGAEA_R1.xlsx", "scripts")     ## [vRN]

# TRIDENT EC10 predictions + the compound -> SMILES map used to join them

pred_csv      <- in_file("TRIDENT_prediction_results_EC10_GRO.csv", "raw_data")   ## [vRN] raw TRIDENT (optional)
map_csv       <- in_file("Danube_TRIDENT_mapping.csv", c("outputs", "danube conc vs TU", "trident"))  ## [vRN]
ec10_fallback <- in_file("TRIDENT_EC10_ngL.csv", c("outputs_updated", "data_for_marvin"))             ## [vRN] processed EC10 fallback

outroot   <- file.path(REPO, "outputs")   ## [repo 18.09.2026] was file.path(proj, "outputs_vRN")
...
mdl_file <- in_file("MDL_CECs_summary.csv", "raw_data")   ## [vRN] download Pedro's Marvin file here
...
.R_DIR <- file.path(REPO, "R")   ## [repo 18.09.2026] REPO is located at the top of this file
